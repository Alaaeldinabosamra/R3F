...
const FollowCamera = ({ targetPosition, cameraZoom }) => {
  const camera = useRef();
  ...
  useFrame(() => {
    if (camera.current) {
      ...
      camera.current.position.lerp({ x: x, y: y + 3, z: z + cameraZoom }, 0.1);
...
const MovingMob = () => {
  ...
  const [cameraZoom, setCameraZoom] = useState(5); // Initial camera zoom value
  ...
  // Scroll event for zooming the camera (inverted behavior)
  const handleWheel = (event) => {
    // Invert zoom direction based on wheel deltaY
    setCameraZoom((prevZoom) => {
      const newZoom = prevZoom + event.deltaY * 0.01; // Inverted zoom
      return THREE.MathUtils.clamp(newZoom, 5, 50); // Clamp zoom between 5 and 50
    });
  };
  ...
  return (
    <>
      ...
      {currentView === 'follow' ? (
        <FollowCamera targetPosition={targetPosition} cameraZoom={cameraZoom} />
      ) : (
        <>
          ...
          <PerspectiveCamera
            ref={cameraRef}
            makeDefault
            position={[lastStopPosition[0], lastStopPosition[1] + 5, lastStopPosition[2] + cameraZoom]}
